...
//            判断key里面有没有包含 . 主要是为了处理那种 com.baidu.qjj 这种字符串
            if (key.contains(".")) {
//                这里是取最后一个.后面的内容作为shortkey ，对于上面那个例子，这里是 qjj
                final String shortKey = getShortName(key);
                if (super.get(shortKey) == null) {
//                    key不存在直接添加
                    super.put(shortKey, value);
                } else {
//                    如果key已经存在，则把value修改成Ambiguity对象
...
//            如果value是二义性的，也抛出异常，所谓二义性就是有两个以上的相同key进入了map
