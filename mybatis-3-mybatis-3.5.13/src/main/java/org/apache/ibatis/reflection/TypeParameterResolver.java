...
//      获取字段的声明类型，对于Test类里面的testField_GenericField方法中调用的 Calculator.class; 这里是T
        Type fieldType = field.getGenericType();
//        获取字段定义所在类的的Class对象，当前field字段在org.apache.ibatis.reflection.typeparam.Calculator定义
        Class<?> declaringClass = field.getDeclaringClass();
//        调用resolveType方法进行后处理
...
//            解析ParameterizedType的类型
            return resolveParameterizedType((ParameterizedType) type, srcType, declaringClass);
        } else if (type instanceof GenericArrayType) {
//            解析GenericArrayType的类型
